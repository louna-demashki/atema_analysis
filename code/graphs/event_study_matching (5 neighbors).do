...
cap log close
log using "$output\Logs\event_study_matching_5", text replace
...
// matching within grade and cohort
	...
	ren math_score_adj score_2022
		...
		save "$input\interim\matched_2022_unr_`g'_5.dta", replace
		restore
		}
		
	use "$input\interim\matched_2022_unr_8_5.dta", clear
	append using "$input\interim\matched_2022_unr_9_5.dta" 
	append using "$input\interim\matched_2022_unr_10_5.dta" 
	save "$input\interim\matched_2022_unr_5.dta", replace 
	...
	keep if grade_21 == `g'		
		...
		ren score_ math_score_adj
		egen m_period = group(match_group year)
		save "$input\interim\matched_2023_unr_`g'_5.dta", replace
		restore
		}
		
	use "$input\interim\matched_2023_unr_8_5.dta", clear
	append using "$input\interim\matched_2023_unr_9_5.dta"    
	save "$input\interim\matched_2023_unr_5.dta", replace 
	
	use "$input\interim\matched_2022_unr_5.dta", clear
	append using "$input\interim\matched_2023_unr_5.dta"
	save "$input\interim\matched_unr_5.dta", replace 
...
use "$input\interim\matched_unr_5.dta", clear
...
graph save Graph "$output\graphs\event_study_unr_match_5 (grade 6).gph", replace
...
graph save Graph "$output\graphs\event_study_unr_match_5 (grade 7).gph", replace
...
graph save Graph "$output\graphs\event_study_unr_match_5 (grade 8).gph", replace
...
graph save Graph "$output\graphs\event_study_unr_match_5 (all).gph", replace
