...
global input "$dir\01. build\output" 
global output "$dir\03. analysis\output"

//cap log close
//log using "$output\Logs\event_study_matching_1", text replace


/*use "$input\master_2017_2023", clear

gen math_s_17 = math_score_adj if ACADEMIC_YEAR_ID_FK == 2017	
gen math_s_18 = math_score_adj if ACADEMIC_YEAR_ID_FK == 2018
gen math_s_19 = math_score_adj if ACADEMIC_YEAR_ID_FK == 2019
gen math_s_22 = math_score_adj if ACADEMIC_YEAR_ID_FK == 2022
gen math_s_23 = math_score_adj if ACADEMIC_YEAR_ID_FK == 2023

bys SMAX_STUDENT_ID: egen score_2017 = max(math_s_17)
bys SMAX_STUDENT_ID: egen score_2018 = max(math_s_18)
bys SMAX_STUDENT_ID: egen score_2019 = max(math_s_19)

sort SMAX_STUDENT_ID ACADEMIC_YEAR_ID_FK

save "$output\temp_data\matching_data", replace 
*/

use "$output\temp_data\matching_data", clear 
...
// matching within grade and cohort
	...
	ren math_score_adj score_2022
		...
		metric(ivar) gen(rn_neighbor_) nneighbor(1) osample(nmatch_all) vce(iid)
		keep SMAX_STUDENT_ID ACADEMIC_YEAR_ID_FK takeup1 rn_neighbor_1 row_num
		keep if takeup1 == 1
		...
		save "$output\temp_data\matched_2022_unr_`g'_1.dta", replace
		restore 
		}
		
	use "$output\temp_data\matched_2022_unr_8_1.dta", clear
	append using "$output\temp_data\matched_2022_unr_9_1.dta" 
	append using "$output\temp_data\matched_2022_unr_10_1.dta" 
	save "$output\temp_data\matched_2022_unr_1.dta", replace 

	
	//cohort 2 (unrestricted matching)
	use "$output\temp_data\matching_data", clear 
	...
	keep if grade_21 == `g'		
		...
		metric(ivar) gen(rn_neighbor_) nneighbor(1) osample(nmatch_all) vce(iid)
		keep SMAX_STUDENT_ID ACADEMIC_YEAR_ID_FK takeup2 rn_neighbor_1 row_num
		keep if takeup2 == 1
		gen match_group = SMAX_STUDENT_ID
		gen rn_neighbor_0 = row_num
		...
		ren score_ math_score_adj
		egen m_period = group(match_group year)
		save "$output\temp_data\matched_2023_unr_`g'_1.dta", replace
		restore
		}
		
	use "$output\temp_data\matched_2023_unr_8_1.dta", clear
	append using "$output\temp_data\matched_2023_unr_9_1.dta"    
	save "$output\temp_data\matched_2023_unr_1.dta", replace 
	
	use "$output\temp_data\matched_2022_unr_1.dta", clear
	append using "$output\temp_data\matched_2023_unr_1.dta"
	save "$output\temp_data\matched_unr_1.dta", replace 
...
use "$output\temp_data\matched_unr_1.dta", clear
...
twoway ///
       ...
       title("Unrestricted (Grade 6)") graphregion(color(white)) ///
...
graph save Graph "$output\graphs\event_study_unr_match_1 (grade 6).gph", replace
...
twoway ///
       ...
       legend(label(1 "95% CI for Take-up in Year 1") label(2 "Coef for Take-up in Year 1") ///
              label(3 "95% CI for Take-up in Year 2") label(4 "Coef for Take-up in Year 2") size(small)) ///
       ytitle("Coefficient and 95% CI") xtitle("Year") ///
       title("Unrestricted (Grade 7)") graphregion(color(white)) ///
       xline(2022, lpattern(dash) lcolor(blue)) ///
       xline(2023, lpattern(dash) lcolor(red)) ///  
	   xlabel(2017(1)2023)
...
graph save Graph "$output\graphs\event_study_unr_match_1 (grade 7).gph", replace
...
twoway ///
       ...
       ytitle("Coefficient and 95% CI") xtitle("Year") ///
       title("Unrestricted (Grade 8)") graphregion(color(white)) ///
       xline(2022, lpattern(dash) lcolor(blue)) ///
       xline(2023, lpattern(dash) lcolor(red)) ///  
	   xlabel(2017(1)2023)
...
graph save Graph "$output\graphs\event_study_unr_match_1 (grade 8).gph", replace
...
twoway ///
       ...
       legend(label(1 "95% CI for Take-up in Year 1") label(2 "Coef for Take-up in Year 1") ///
              label(3 "95% CI for Take-up in Year 2") label(4 "Coef for Take-up in Year 2") size(small)) ///
       ytitle("Coefficient and 95% CI") xtitle("Year") ///
       title("Unrestricted (Grades 6-8)") graphregion(color(white)) ///
       xline(2022, lpattern(dash) lcolor(blue)) ///
       xline(2023, lpattern(dash) lcolor(red)) ///  
	   xlabel(2017(1)2023)
...
graph save Graph "$output\graphs\event_study_unr_match_1 (all).gph", replace
