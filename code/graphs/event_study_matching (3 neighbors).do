...
cap log close
log using "$output\Logs\event_study_matching_3", text replace

use "$input\interim\matching_data", clear 
...
// matching within grade and cohort
	...
	ren math_score_adj score_2022
		...
		metric(ivar) gen(rn_neighbor_) nneighbor(3) osample(nmatch_all) vce(iid)
		keep SMAX_STUDENT_ID ACADEMIC_YEAR_ID_FK takeup1 rn_neighbor_1-rn_neighbor_3 row_num
		keep if takeup1 == 1
		...
		save "$input\interim\matched_2022_unr_`g'_3.dta", replace
		restore
		}
		
	use "$input\interim\matched_2022_unr_8_3.dta", clear
	append using "$input\interim\matched_2022_unr_9_3.dta" 
	append using "$input\interim\matched_2022_unr_10_3.dta" 
	save "$input\interim\matched_2022_unr_3.dta", replace 
	...
	keep if grade_21 == `g'		
		...
		metric(ivar) gen(rn_neighbor_) nneighbor(3) osample(nmatch_all) vce(iid)
		keep SMAX_STUDENT_ID ACADEMIC_YEAR_ID_FK takeup2 rn_neighbor_1-rn_neighbor_3 row_num
		keep if takeup2 == 1
		gen match_group = SMAX_STUDENT_ID
		gen rn_neighbor_0 = row_num
		...
		ren score_ math_score_adj
		egen m_period = group(match_group year)
		save "$input\interim\matched_2023_unr_`g'_3.dta", replace
		restore 
		}
		
	use "$input\interim\matched_2023_unr_8_3.dta", clear
	append using "$input\interim\matched_2023_unr_9_3.dta"    
	save "$input\interim\matched_2023_unr_3.dta", replace 
		
	use "$input\interim\matched_2022_unr_3.dta", clear
	append using "$input\interim\matched_2023_unr_3.dta"
	save "$input\interim\matched_unr_3.dta", replace 
...
use "$input\interim\matched_unr_3.dta", clear
...
graph save Graph "$output\graphs\event_study_unr_match_3 (grade 6).gph", replace
...
graph save Graph "$output\graphs\event_study_unr_match_3 (grade 7).gph", replace
...
graph save Graph "$output\graphs\event_study_unr_match_3 (grade 8).gph", replace
...
graph save Graph "$output\graphs\event_study_unr_match_3 (all).gph", replace
