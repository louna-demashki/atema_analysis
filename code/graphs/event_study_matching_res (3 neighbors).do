...
cap log close
log using "$output\Logs\event_study_matching_res_3", text replace
...
// matching within grade and cohort
	...
	keep if ACADEMIC_YEAR_ID_FK == 2022 & inrange(grade_21,8,10) 
		...
		save "$input\interim\matched_2022_res_`g'_3.dta", replace
		restore
		}
		
	use "$input\interim\matched_2022_res_8_3.dta", clear
	append using "$input\interim\matched_2022_res_9_3.dta"
	append using "$input\interim\matched_2022_res_10_3.dta"    
	save "$input\interim\matched_2022_res_3.dta", replace 
	...
	keep if grade_21 == `g'		
		...
		ren score_ math_score_adj
		egen m_period = group(match_group year)
		save "$input\interim\matched_2023_res_`g'_3.dta", replace
		restore
		}
		
	use "$input\interim\matched_2023_res_8_3.dta", clear
	append using "$input\interim\matched_2023_res_9_3.dta"    
	save "$input\interim\matched_2023_res_3.dta", replace 
	
	use "$input\interim\matched_2022_res_3.dta", clear
	append using "$input\interim\matched_2023_res_3.dta"
	save "$input\interim\matched_res_3.dta", replace 
...
use "$input\interim\matched_res_3.dta", clear
...
graph save Graph "$output\graphs\event_study_res_match_3 (grade 6).gph", replace
...
graph save Graph "$output\graphs\event_study_res_match_3 (grade 7).gph", replace
...
graph save Graph "$output\graphs\event_study_res_match_3 (grade 8).gph", replace
...
graph save Graph "$output\graphs\event_study_res_match_3 (all).gph", replace
