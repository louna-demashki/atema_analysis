...
cap log close
log using "$output\Logs\event_study_matching_res_1", text replace
...
keep if takeup1 == 1 | takeup2 == 1 | control == 1

save "$input\interim\matching_data_res", replace 
*/
...
// matching within grade and cohort
	...
	ren math_score_adj score_2022
		...
		teffects nnmatch (score_2022 ${score_vars}) (takeup1), ate ///
		...
		save "$input\interim\matched_2022_res_`g'_1.dta", replace
		restore
		}
		
	use "$input\interim\matched_2022_res_8_1.dta", clear
	append using "$input\interim\matched_2022_res_9_1.dta"
	append using "$input\interim\matched_2022_res_10_1.dta"    
	save "$input\interim\matched_2022_res_1.dta", replace 
	...
	keep if grade_21 == `g'		
		...
		ren score_ math_score_adj
		egen m_period = group(match_group year)
		save "$input\interim\matched_2023_res_`g'_1.dta", replace	
		restore
		}
		
	use "$input\interim\matched_2023_res_8_1.dta", clear
	append using "$input\interim\matched_2023_res_9_1.dta"    
	save "$input\interim\matched_2023_res_1.dta", replace 
		
	use "$input\interim\matched_2022_res_1.dta", clear
	append using "$input\interim\matched_2023_res_1.dta"
	save "$input\interim\matched_res_1.dta", replace 
...
use "$input\interim\matched_res_1.dta", clear
...
graph save Graph "$output\graphs\event_study_res_match_1 (grade 6).gph", replace
...
graph save Graph "$output\graphs\event_study_res_match_1 (grade 7).gph", replace
...
graph save Graph "$output\graphs\event_study_res_match_1 (grade 8).gph", replace
...
graph save Graph "$output\graphs\event_study_res_match_1 (all).gph", replace
